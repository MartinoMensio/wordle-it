function share_custom_func(e) {
    var shadow = document.getElementsByTagName('game-app')[0].shadowRoot.querySelector('game-theme-manager').getElementsByTagName('game-settings')[0].shadowRoot
    var val = shadow.querySelector('#share_custom_text').value
    var link = '?custom=' + btoa(val)
        // navigator.clipboard.writeText(val);
    shadow.querySelector('#share_custom_url').text = 'link'
    shadow.querySelector('#share_custom_url').href = link
}
this.wordle = this.wordle || {}, this.wordle.bundle = function(e) {
    ...
    function Pa(e) {
        var a, s = Na(e);
        var key = new URLSearchParams(window.location.search).get('custom')
        if (q) {
            return atob(key)
        } else {
